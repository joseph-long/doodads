    ...
    if overwrite or not os.path.exists(filepath):
        ...
        while retries > 0:
            try:
                ...
                log.warn(
                    f"(attempt {attempt_num} / {DOWNLOAD_RETRIES}) Failed to retrieve {url}, exception was {e}"
                )
                retries -= 1
                if retries > 0:
        ...
        log.info(
            f"Existing download for {url} -> {filepath}, pass overwrite=True to replace"
        )
    return filepath
...
class RemoteResource(BaseResource):
    ...
    def ensure_exists(self):
        if not self.exists:
            log.info(
                f"Resource not yet generated at output file path {self.output_filepath}"
            )
            self.download()
            log.info(
                f"Converting {self.url} -> {self.output_filepath} with {self.converter_function}"
            )
            self.convert()
        else:
...
class CollectionResource(BaseResource):
    ...
    def convert(self):
        for res in self.resources:
            res.ensure_exists()
        self.converter_function(
            [res.output_filepath for res in self.resources], self.output_filepath
        )
...
class RemoteResourceRegistry:
    ...
    def add_from_url(
        self, module, url, converter_function=os.symlink, output_filename=None
    ):
...
def execution_environment() -> str:
    try:
        ...
        try:
            ...
            if isinstance(ip, zmqshell.ZMQInteractiveShell):
                return "notebook"
        except ImportError:
            pass
        try:
        ...
        # any other error, including no IPython import
        pass
    return "terminal"
