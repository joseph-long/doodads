...
from ..modeling.units import FLUX_PER_FREQUENCY_UNITS, FLUX_UNITS, WAVELENGTH_UNITS
...
__all__ = [
    ...
    "BOBCAT_SPECTRA_Mminus0_5",
    "BOBCAT_EVOLUTION_Mminus0_5",
    "BOBCAT_EVOLUTION_TABLES_Mminus0_5",
    "BOBCAT_SPECTRA_Mplus0_5",
    "BOBCAT_EVOLUTION_Mplus0_5",
    "BOBCAT_EVOLUTION_TABLES_Mplus0_5",
]
...
BOBCAT_PHOTOMETRY_MAG_COLS = _BOBCAT_PHOTOMETRY_COLS + [
    f"mag_{band}" for band in _BOBCAT_PHOTOMETRY_BANDS
]
BOBCAT_PHOTOMETRY_FLUX_COLS = _BOBCAT_PHOTOMETRY_COLS + [
    f"log_flux_mJy_{band}" for band in _BOBCAT_PHOTOMETRY_BANDS
]
...
def read_bobcat(fh, colnames, first_header_line_contains):
    ...
    for line in fh:
        ...
        for idx, col in enumerate(colnames):
            try:
                val = utils.FLOAT_RE.findall(parts[idx])[0]
                val = float(val)
...
BOBCAT_2021_EVOLUTION_PHOTOMETRY_ARCHIVE = (
...
load_bobcat_evolution_age = partial(
    _load_from_resource, BOBCAT_EVOLUTION_AGE_COLS, "age(Gyr)"
)
load_bobcat_evolution_mass = partial(
    _load_from_resource, BOBCAT_EVOLUTION_MASS_COLS, "age(Gyr)"
)
load_bobcat_photometry_mag = partial(
    _load_from_resource, BOBCAT_PHOTOMETRY_MAG_COLS, "Teff"
)
load_bobcat_photometry_flux = partial(
    _load_from_resource, BOBCAT_PHOTOMETRY_FLUX_COLS, "Teff"
)
...
def load_bobcat_spectrum(
    fh,
    source_wavelength_unit=u.um,
    source_flux_per_frequency_unit=(u.erg / u.cm**2 / u.s / u.Hz),
    wavelengths=None,
):
    header = next(fh)
    (T_eff_K, gravity_m_per_s2, Y, f_rain, Kzz, Fe_over_H, C_over_O, f_hole) = tuple(
        map(float, FLOAT_RE.findall(header))
    )
    ...
    these_wavelengths = (
        these_wavelengths[wavelength_order_sorter] * source_wavelength_unit
    ).to(WAVELENGTH_UNITS)
    if wavelengths is not None and not np.all(
        (these_wavelengths - wavelengths) / wavelengths < 5e-7
    ):
    ...
    these_fluxes = (
        these_fluxes[wavelength_order_sorter] * source_flux_per_frequency_unit
    )
...
def _convert_spectra(
    tarfile_filepath, output_filepath, match_pattern=BOBCAT_SPECTRA_FILENAMES
):
    archive_tarfile = tarfile.open(tarfile_filepath)
    spectra_paths = [
        name for name in archive_tarfile.getnames() if match_pattern.match(name)
    ]
    if len(spectra_paths) == 0:
        raise RuntimeError("Could not match any spectra names")
    params_tbl = np.zeros(
        len(spectra_paths), dtype=list((param, float) for param in SPECTRA_PARAMS_COLS)
    )
    wavelengths = None
    spectra = None
    for idx, name in enumerate(spectra_paths):
        ...
        try:
            ...
            raise InconsistentSamplingException(
                f"Inconsistent wavelength sampling in {name}"
            )
        finally:
            rawfh.close()
        if spectra is None:
            ...
            spectra: u.Quantity = (
                np.zeros((len(spectra_paths), len(f_lambda))) * FLUX_UNITS
            )
    ...
    with open(output_filepath, "wb") as fh:
        hdul = fits.HDUList(
            [
                ...
                fits.ImageHDU(
                    data=wavelengths.to(WAVELENGTH_UNITS).value, name="WAVELENGTHS"
                ),
...
class BobcatModelSpectraGrid(model_grids.ModelSpectraGrid):
    ...
    def _interpolate(self, T_eff_K=None, gravity_m_per_s2=None, **kwargs):
        ...
        if (
            T_eff_K < min_T_K
            and np.abs((T_eff_K - min_T_K) / min_T_K) < self.fractional_param_err
        ):
            print(f"replacing {T_eff_K=} with {min_T_K=}")
            kwargs["T_eff_K"] = min_T_K
        elif (
            T_eff_K > max_T_K
            and np.abs((T_eff_K - max_T_K) / max_T_K) < self.fractional_param_err
        ):
        ...
        if (
            gravity_m_per_s2 < min_g_m_per_s2
            and np.abs((gravity_m_per_s2 - min_g_m_per_s2) / min_g_m_per_s2)
            < self.fractional_param_err
        ):
            kwargs["gravity_m_per_s2"] = min_g_m_per_s2
        elif (
            gravity_m_per_s2 > max_g_m_per_s2
            and np.abs((gravity_m_per_s2 - max_g_m_per_s2) / max_g_m_per_s2)
            < self.fractional_param_err
        ):
...
BOBCAT_SPECTRA_Mplus0_5 = BobcatModelSpectraGrid(
    BOBCAT_2021_SPECTRA_Mplus0_5_DATA.output_filepath
)
BOBCAT_SPECTRA_Mminus0_5 = BobcatModelSpectraGrid(
    BOBCAT_2021_SPECTRA_Mminus0_5_DATA.output_filepath
)
...
class BobcatEvolutionTables(utils.LazyLoadable):
    ...
    def _lazy_load(self):
        ...
        if not np.any(
            np.isclose(self.metallicity, [k for k in self._metallicity_to_string])
        ):
            raise ValueError(
                f"Available metallicities are {[v for v in self._metallicity_to_string.values()]}"
            )
...
BOBCAT_EVOLUTION_TABLES_M0 = BobcatEvolutionTables(
    BOBCAT_2021_EVOLUTION_PHOTOMETRY_DATA.output_filepath, metallicity=0.0
)
BOBCAT_EVOLUTION_TABLES_Mplus0_5 = BobcatEvolutionTables(
    BOBCAT_2021_EVOLUTION_PHOTOMETRY_DATA.output_filepath, metallicity=0.5
)
...
class BobcatEvolutionModel:
    _interp_mass_temp_to_log_g: typing.Callable[
        [np.ndarray, np.ndarray], np.ndarray
    ] = None
    _interp_mass_age_to_T_evol: typing.Callable[
        [np.ndarray, np.ndarray], np.ndarray
    ] = None
        ...
        if self._interp_mass_age_to_T_evol is None:
            self._interp_mass_age_to_T_evol = interpolate.LinearNDInterpolator(
                np.stack(
                    [
                        self.evolution_tables.mass["mass_Msun"],
                        self.evolution_tables.mass["age_Gyr"],
                    ],
                    axis=-1,
                ),
                self.evolution_tables.mass["T_eff_K"],
    ...
    def mass_age_to_properties(
        ...
        surface_gravity_m_per_s2 = (
            (10**log_g_cm_per_s2_vals * u.cm / u.s**2).to(u.m / u.s**2).value
        )
        ...
        for idx in range(len(T_eff)):
            if not np.isnan(T_eff[idx]) and not np.isnan(surface_gravity[idx]):
                try:
                    ...
                    mag_value = convolved_mag_ref.magnitude(
                        spec.multiply(filter_spectrum)
                    )
                except model_grids.BoundsError as e:
                    log.debug(
                        f"Out of bounds {T_eff[idx]}, {surface_gravity[idx]}, {mass[idx]}"
                    )
                    mag_value = np.nan
                mags[idx] = mag_value
            else:
                log.debug(
                    f"Nonfinite interpolated values {T_eff[idx]}, {surface_gravity[idx]}, {mass[idx]}"
                )
                mags[idx] = np.nan
        if mass_scalar and age_scalar:
            T_evol, T_eff, surface_gravity, mags = (
                T_evol[0],
                T_eff[0],
                surface_gravity[0],
                mags[0],
    ...
    def magnitude_age_to_mass(
        ...
        subset_masses, subset_mags, excluded_ranges = math.make_monotonic_decreasing(
            tabulated_masses, mags
        )
        TOO_FAINT = -np.inf
        TOO_BRIGHT = np.inf
        mass_Mjup = interpolate.interp1d(
            subset_mags,
            subset_masses.to(u.Mjup).value,
            bounds_error=False,
            fill_value=(TOO_BRIGHT, TOO_FAINT),
        )(magnitude)
...
BOBCAT_EVOLUTION_M0 = BobcatEvolutionModel(
    BOBCAT_EVOLUTION_TABLES_M0, BOBCAT_SPECTRA_M0
)
BOBCAT_EVOLUTION_Mplus0_5 = BobcatEvolutionModel(
    BOBCAT_EVOLUTION_TABLES_Mplus0_5, BOBCAT_SPECTRA_Mplus0_5
)
BOBCAT_EVOLUTION_Mminus0_5 = BobcatEvolutionModel(
    BOBCAT_EVOLUTION_TABLES_Mminus0_5, BOBCAT_SPECTRA_Mminus0_5
)
